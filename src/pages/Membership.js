...
function Membership() {
  ...
  function makeFileObjects(data) {
    ...
    const files = [new File([blob], "Membership_Details.json")];
    return files;
  }
  ...
  const createMembership = async (data) => {
    ...
    try {
      ...
      let files = makeFileObjects(data);
      let cid = await storage(files);
      ...
      membership.set(
        "storageURL",
        `https://${cid}.ipfs.dweb.link/Membership_Details`
      );
      membership.save();
  ...
  return (
    <Page title="Membership |  DAO Tool">
